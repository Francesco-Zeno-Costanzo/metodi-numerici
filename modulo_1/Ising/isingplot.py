...
#i seguenti dati sono stati presi in un reticolo 50x50 tra beta 0.42 e 0.45
e50, c50, de50, dc50 = np.loadtxt(r'datiplot50picco.dat', unpack=True)

#seleziono solo un range dei dati
y = c50[21:34]
dy = dc50[21:34]

#array dlle termperature
Bb = np.zeros(len(c50))
for i in range(len(Bb)):
    Bb[i] = 0.42 + i*(0.45-0.42)/len(Bb)
Bb = abs(Bb[21:34]-bc)

#valori che mi aspetto per i parametri ottimali
init = np.array([0.1, 0, 10.75]) #aiutano la convergenza del fit

pars3, covm3 = curve_fit(FX, Bb, y, sigma=dy, absolute_sigma=False)
print(f'm = {pars3[0]:.5f} +- {np.sqrt(covm3.diagonal()[0]):.5f}')
print(f'a = {pars3[1]:.5f} +- {np.sqrt(covm3.diagonal()[1]):.5f}')
print(f'q = {pars3[2]:.5f} +- {np.sqrt(covm3.diagonal()[2]):.5f}')


chi4, Nd4 = ChiRes(y, dy, Bb, pars3)
...
frame1 = fig1.add_axes((.1, .35, .8, .6))
#frame1=fig1.add_axes((trasla lateralmente, trasla verticamente, larghezza, altezza))
frame1.set_title('Calore specifico intorno a punto critico reticolo 50x50', fontsize=20)
frame1.set_ylabel('Calore specifico [a.u.]', fontsize=15)
plt.grid()

plt.errorbar(Bb, y, dy, fmt='.', color='black', label='dati') 
t=np.linspace(np.min(Bb), np.max(Bb), 10000)

plt.plot(t, FX(t, *pars3), color='blue', label='best fit') 
plt.legend(loc='best')
...
ff = ChiRes(y, dy, Bb, pars3, 1)
x1 = np.linspace(np.min(Bb), np.max(Bb), 1000)

plt.plot(x1, 0*x1, color='red', linestyle='--', alpha=0.5)
plt.plot(Bb, ff, '.', color='black') 
plt.grid()
